...
def push(**kwargs):
    ...
    ti = kwargs['ti']
    return ti.xcom_push(key='value from pusher 1', value=value_1)
...
def puller(**kwargs):
    ...
    if pulled_value_1 != value_1:
        logging.debug(f"airflow.task >>> pulled_value_1 - {pulled_value_1}")
        raise ValueError(f'The two values differ {pulled_value_1} and {value_1}')

    # get value_2
    pulled_value_2 = ti.xcom_pull(task_ids='push_by_returning')
    if pulled_value_2 != value_2:
         logging.debug(f"airflow.task >>> pulled_value_2 - {pulled_value_2}")
